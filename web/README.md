...
构建时会从 Markdown 内容生成静态 JSON，供微信小程序或其他客户端读取：
...
- `/api/manifest.json`：站点与集合元信息
- `/api/garden.json`：博客与随笔混合内容流
- `/api/posts.json`：技术博客列表
- `/api/posts/[slug].json`：技术博客详情
- `/api/essays.json`：生活随笔列表
- `/api/essays/[slug].json`：生活随笔详情
- `/api/tags.json`：标签聚合

部署后可通过 `https://blog.nianshu2022.cn/api/garden.json` 访问。
...
│   ├── generate-miniapp-api.mjs # 小程序静态 JSON API 生成
